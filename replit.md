...
- Auth: JWT HS256 30d (`jsonwebtoken`), bcrypt 12; Google web OAuth + native Google/Apple token sign-in verified against provider JWKS via `jose`; soft email verification + password reset via Resend (`lib/email.ts` — sha256 tokens in users table, links logged when RESEND_API_KEY unset). **Email MFA on password login**: `/auth/login` verifies the password then emails a 6-digit code (10-min expiry, single-use, `login_verification_code/_expires` cols); the session token is only issued by `/auth/verify-login-code`. Web + mobile both run the two-step flow. ⚠️ RESEND_API_KEY is login-critical (unset → code only in logs). Google/Apple sign-in skips the code.
...
- `artifacts/api-server/src/routes/` — `auth` (register/login [+ **verify-login-code** email MFA]/oauth/me [GET + **PATCH profile**]/delete/verify-email/reset/pin), `notifications` (list/mark-read; helper `lib/notify.ts` — notifyUser/notifyAll, called from sends/webhooks/admin), `enquiries` (public POST, rate-limited), `kyc` (start), `wallet` (chain balances, P2P sends by phone/email/**S-PAY ID**/address, deposit instructions), `banking` (accounts/rates/withdraw — Noah-gated), `card` (details/issue/waitlist — flag-gated), `jobs` (+ `/sitemap.xml`), `ssr` (bot-rendered job pages), `admin` (stats/users/transactions/feature-flags/fees/custom-jobs/settings), `webhooks` (Noah KYC/KYB + deposits, Stripe), `health` (`/healthz`, `/status`)
