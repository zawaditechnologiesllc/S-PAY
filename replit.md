...
- **Transaction PIN** (`lib/pin.ts`): bcrypt-hashed 4–6 digit PIN in users.transaction_pin_hash, REQUIRED on every send (`/wallet/send`) and withdrawal (`/banking/withdraw`) — the login JWT alone never moves money. 5 wrong tries → 15-min lockout (atomic SQL increment); `/auth/pin` sets/changes it (change needs currentPin). API returns hasPin; never the hash. Errors: pin_not_set(428)/pin_locked(423)/pin_wrong(401).
...
- **Send recipient resolution**: phone OR email (both → DB lookup, self-send blocked, recipient wallet JIT-provisioned) OR raw 0x address.
