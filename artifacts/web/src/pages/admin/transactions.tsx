        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-gray-900">{isLoading ? "…" : `${data?.total ?? 0} transactions`}</span>
            </div>
                ...
                {["all", "receive", "send", "withdraw", "payment"].map((s) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all capitalize ${
                      typeFilter === s ? "bg-white text-[#1A2B4A] shadow-sm" : "text-gray-500 hover:text-gray-700"
                    }`}
                  >
                    {s}
                  </button>
                  ...
                  txs.map((t: any) => {
                    ...
                    return (
                      <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors">
                        ...
                        <td className="px-5 py-3.5 font-medium text-gray-900">{t.description}</td>
                        <td className="px-5 py-3.5 text-gray-500">{t.counterparty ?? "—"}</td>
                        <td className={`px-5 py-3.5 font-semibold ${TYPE_COLOR[t.type] ?? "text-gray-900"}`}>
                          {isIn ? "+" : "-"}{Math.abs(t.amount).toFixed(2)}
                        </td>
                        <td className="px-5 py-3.5 text-gray-500">{t.currency}</td>
                        <td className="px-5 py-3.5">
                        ...
                        <td className="px-5 py-3.5 text-gray-500 whitespace-nowrap">{new Date(t.createdAt).toLocaleDateString()}</td>
                      </tr>
                    );
                  })
                )}
