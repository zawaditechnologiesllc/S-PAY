...
| **Withdraw — grouped methods** — Mobile money expands into M-Pesa/MTN MoMo/Airtel/GCash/Nequi; PIX, SEPA, UK Faster Payments, SPEI, US/Intl bank top-level; dashboard Mobile-money tile offers Top up AND Withdraw | ✅ Live | web `pages/withdraw.tsx`, `pages/dashboard.tsx` |
...
| **Transaction PIN** — bcrypt 4–6 digit PIN required on **every money-out flow**: P2P send, exchange/crypto cash-out, and bank/mobile-money withdrawal (login JWT alone never moves money); 5 tries → 15-min lockout; set/change in Security & PIN (web + mobile collect and pass it on all four flows) | ✅ Live | `lib/pin.ts`, `routes/auth.ts` (`/auth/pin`), `routes/wallet.ts`, `routes/banking.ts`, web `pages/security.tsx` + `exchange-withdraw.tsx` + `withdraw.tsx` + `components/quick-actions.tsx`, mobile `app/security.tsx` + `app/withdraw.tsx` + `(tabs)/index.tsx` |
| **Profile editing** — name, phone (unique), country, business name editable from the Profile screen; persists to the user record (`PATCH /auth/me`), avatar URLs validated, phone collisions blocked | ✅ Live | `routes/auth.ts` (`PATCH /auth/me`), web `pages/profile.tsx`, mobile `app/profile.tsx` |
| **Transfer commission (your revenue)** — flat + % fee per send, charged on top, collected to `TREASURY_CELO_ADDRESS`; admin-set in Settings → Fees; free until a treasury wallet is configured | ✅ Live | `settings.transferFee`, `routes/wallet.ts` |
| **Send by phone / email / address** — P2P resolves an S-PAY member by phone OR email, or sends to any Celo address | ✅ Live | `routes/wallet.ts`, web `components/quick-actions.tsx` |
| **SocialConnect (Celo identity)** — optional, additive: resolve phone/email → address via Celo's decentralized registry so sends can reach non-members. Admin switch + flag + send-flow fallback are wired; a no-op until issuer keys are set and the resolver is implemented (NOT a WaaS replacement, doesn't cut wallet cost) | ⚙️ Scaffold + switch live — activate by following `docs/SOCIALCONNECT.md` (set issuer env keys, implement §4/§5) | `lib/socialconnect.ts`, `lib/settings.ts`, `routes/wallet.ts`, Admin → Settings → SocialConnect |
...
| **Auto-migrations** — schema applies itself on every boot (14 migrations, 0000–0013) | ✅ Live | `lib/db/migrations/`, `src/lib/migrate.ts` |
...
| **D5** | ✅ **Done** — full reset flow live (`/auth/forgot-password` + `/auth/reset-password`, success page, redirect) + **email confirmation at sign-up** (soft, banner + resend). Set `RESEND_API_KEY` (+ `EMAIL_FROM`) to actually send; until then links print in Render logs | New: `POST /auth/forgot-password` + `POST /auth/reset-password` in `routes/auth.ts`; wire `web/src/pages/forgot-password.tsx`; needs an email provider (e.g. Resend) + reset-token column/table | Until built, the page is a dead end — either build or temporarily link "Contact support" instead. |
| **D6** | ✅ **Done** — all four quick actions work: Scan (camera QR reader), Transfer (P2P phone / any address), Recharge (your shareable QR + address), Withdraw (cash-out flows) | `web/src/pages/dashboard.tsx` (`QuickAction`) | Link: Transfer → P2P send UI (build a small send dialog calling `POST /wallet/send`), Recharge → wallet deposit (add-funds modal showing the Celo address), Withdraw → `/banking/withdraw`, Scan → QR scan (mobile-first, can hide on web). |
...
`DATABASE_URL` · `JWT_SECRET` · `SESSION_SECRET` · `CORS_ORIGIN` · `ADMIN_EMAILS` · `SITE_URL` · `PRIVY_APP_ID` · `PRIVY_APP_SECRET` · `CDP_API_KEY_ID` · `CDP_API_KEY_SECRET` · `CDP_WALLET_SECRET` · `TURNKEY_API_PUBLIC_KEY` · `TURNKEY_API_PRIVATE_KEY` · `TURNKEY_ORGANIZATION_ID` · `OPENFORT_API_KEY` · `OPENFORT_WALLET_SECRET` · `THIRDWEB_SECRET_KEY` · `THIRDWEB_API_BASE` · `DYNAMIC_ENVIRONMENT_ID` · `DYNAMIC_API_TOKEN` · `DYNAMIC_WALLET_PASSWORD_SECRET` · `NOAH_API_KEY` · `NOAH_WEBHOOK_SECRET` · `RESEND_API_KEY` · `EMAIL_FROM` · `STRIPE_SECRET_KEY` · `STRIPE_WEBHOOK_SECRET` · `GOOGLE_CLIENT_ID` · `GOOGLE_CLIENT_SECRET` · `GOOGLE_ANDROID_CLIENT_ID` · `GOOGLE_IOS_CLIENT_ID` · `APPLE_BUNDLE_ID` · `FRONTEND_URL` · `API_BASE_URL` · `CELO_RPC_URL` · `CELO_CAIP2` · `PRIVY_API_BASE` · `TURNKEY_API_BASE` · `DATABASE_SSL` · `CARD_PROGRAM_ADDRESS_*` · `REMOTIVE_AFFILIATE_URL` · `REMOTE_COM_AFFILIATE_URL` · `MIGRATIONS_DIR` · `TREASURY_CELO_ADDRESS` (collects transfer commissions; unset = transfers free) · `SOCIALCONNECT_ISSUER_ADDRESS` · `SOCIALCONNECT_ISSUER_PRIVATE_KEY` · `SOCIALCONNECT_REGISTRY_ADDRESS` (all optional — Celo SocialConnect identity resolution; see `docs/SOCIALCONNECT.md`) · **SEO engine** (all optional; see `docs/SEO-ENGINE.md`): `GEMINI_API_KEY` (Gemini 2.5 Flash blog drafting — free tier, key from aistudio.google.com/apikey) · `SEO_DRAFT_MODEL` (default `gemini-2.5-flash`) · `GSC_SERVICE_ACCOUNT_JSON` · `GSC_SITE_URL` · `GA4_PROPERTY_ID` · `GA4_SERVICE_ACCOUNT_JSON` · `SEO_REDDIT_ENABLED` · `SEO_REDDIT_SUBREDDITS` · `SEO_REDDIT_USER_AGENT` — plus Vercel `VITE_API_URL`, EAS `EXPO_PUBLIC_API_URL`, `EXPO_PUBLIC_GOOGLE_*_CLIENT_ID`.
