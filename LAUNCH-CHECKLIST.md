...
| **Transaction PIN** — bcrypt 4–6 digit PIN required on every send & withdrawal (login JWT alone never moves money); 5 tries → 15-min lockout; set/change in Security & PIN | ✅ Live | `lib/pin.ts`, `routes/auth.ts` (`/auth/pin`), `routes/wallet.ts`, web `pages/security.tsx`, mobile `app/security.tsx` |
...
| **Auto-migrations** — schema applies itself on every boot (13 migrations) | ✅ Live | `lib/db/migrations/`, `src/lib/migrate.ts` |
